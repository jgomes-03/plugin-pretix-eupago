...
def _handle_webhook_v2(request, event_data, event_body):
    ...
    if 'data' in event_data and isinstance(event_data['data'], str):
        ...
        webhook_secret = _get_webhook_secret_for_decryption(event_data)
        if not webhook_secret:
            logger.warning('No webhook secret available for decryption')
        
        # Try to decrypt with available webhook secrets
        decrypted_data = None
        
        # Get all possible webhook secrets to try for decryption
        webhook_secrets_to_try = []
        if webhook_secret:
            webhook_secrets_to_try.append(webhook_secret)
        
        # Try decryption with each available webhook secret
        for secret in webhook_secrets_to_try:
            decrypted_data = _decrypt_webhook_data(event_data['data'], iv=iv, webhook_secret=secret)
            if decrypted_data:
...
def _get_webhook_secret_for_decryption(event_data):
    """Get webhook secret for decryption - try all possible secrets since we don't know the payment method yet"""
    from django_scopes import scopes_disabled
    import os
    ...
    try:
        with scopes_disabled():
            # Get all organizers that might have webhook secrets configured
            organizers = Organizer.objects.all()
            for organizer in organizers:
                try:
                    # Try to get the default webhook secret first
...
def _get_webhook_secret_for_payment(payment):
    ...
    try:
        ...
        logger.error(f"Error getting webhook secret for payment {payment.full_id}: {e}")
        return None


@csrf_exempt
def debug_webhook_secret(request):
    """
    Debug endpoint for testing webhook secret validation.
    ...
    try:
        ...
        test_signature = data.get('signature', '')
        test_secret = data.get('secret', '')
        
        if not all([test_payload, test_signature, test_secret]):
            return JsonResponse({
                'error': 'Missing required fields: payload, signature, secret'
            }, status=400)
        
        # Create a temporary EuPagoPayByLink instance to test signature validation
        from pretix.base.models import Event
        from .payment import EuPagoPayByLink
        
        try:
            # Get any event for testing (in a real implementation, you'd want to specify the event)
            event = Event.objects.first()
            if not event:
                return JsonResponse({
                    'error': 'No events found for testing'
                }, status=500)
                
            provider = EuPagoPayByLink(event)
            
            # Test the signature validation
            result = provider.test_webhook_signature_validation(test_payload, test_signature, test_secret)
            ...
            return JsonResponse({
                'error': f'Error testing signature: {str(e)}'
            }, status=500)
